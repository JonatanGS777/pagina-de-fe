...
/* ---------- Luces ---------- */
scene.add(new THREE.HemisphereLight(0x9fc0ec, 0x6b5a3a, 0.55));
...
scene.add(fill);
const interior = new THREE.PointLight(0xffd9a0, 30, 60, 2);
interior.position.set(0, 8, -18);
scene.add(interior);
const interior2 = new THREE.PointLight(0xffd9a0, 22, 60, 2);
interior2.position.set(0, 9, 10);
scene.add(interior2);
const interior3 = new THREE.PointLight(0xffe3b0, 14, 50, 2);
interior3.position.set(0, 8, 35);
scene.add(interior3);
...
function tickWalk(dt) {
  ...
  camera.position.x = Math.max(-74, Math.min(74, camera.position.x));
  camera.position.z = Math.max(-74, Math.min(74, camera.position.z));
...
function animate() {
  ...
  for (let i = 0; i < pos.length; i += 3) {
    pos[i + 1] += dt * 0.45;
    if (pos[i + 1] > 26) pos[i + 1] = world.dust.base[i + 1] * 0.9;
    pos[i] += dt * 0.12;
    if (pos[i] > 85) pos[i] = -85;
  }
...
if (document.getElementById("parts-grid")) {
  const GRID = [
    { cat: "exterior", ids: ["temple", "temple-roof", "ulam", "chambers", "pillars", "altar", "sea", "lavers", "court-inner", "court-great", "gates"] },
    { cat: "interior", ids: ["hekal", "debir", "veil", "incense", "table", "menorot", "ark", "cherubim"] },
    { cat: "mobiliario", ids: ["pillars", "altar", "sea", "lavers", "incense", "table", "menorot", "ark", "cherubim"] },
  ];
...
if (new URLSearchParams(location.search).has("selftest")) {
  ...
  setTimeout(() => selectPart("debir", true), 1000);
  setTimeout(() => selectPart("sea", true), 2600);
  setTimeout(() => {
