...
export function createUI(api) {
  ...
  try {const saved=JSON.parse(localStorage.getItem('templo-salomon-progreso-v1')||'[]');if(Array.isArray(saved))saved.filter(id=>byId[id]).forEach(id=>visited.add(id));}catch{/* Storage can be unavailable in private browsing. */}
  ...
  function select(id,learn=false) {
    ...
    visited.add(id);try{localStorage.setItem('templo-salomon-progreso-v1',JSON.stringify([...visited]));}catch{}
