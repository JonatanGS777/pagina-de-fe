...
import { zones, zoneAt, move, canStand } from './navigation.js';
...
function start() {
  const stage=$('stage');const scene=new T.Scene();scene.background=new T.Color('#d5d3be');scene.fog=new T.Fog('#d5d3be',70,280);
  ...
  scene.add(new T.HemisphereLight('#fcf9e9','#8c7652',2.3));
  const sun=new T.DirectionalLight('#fff0d4',3.2);sun.position.set(-32,48,28);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);
  Object.assign(sun.shadow.camera,{left:-36,right:36,top:38,bottom:-38,near:1,far:110});sun.shadow.normalBias=.035;sun.shadow.bias=-.0002;scene.add(sun);
  ...
  const inner=new T.PointLight('#ffe5ab',9,18,2);inner.position.set(0,3.6,-14);scene.add(inner);
  const world=createWorld(scene);
  const camera=new T.PerspectiveCamera(48,1,.06,700);camera.position.fromArray(zones.exterior.view);
  const controls=new OrbitControls(camera,renderer.domElement);controls.target.fromArray(zones.exterior.target);controls.enableDamping=true;controls.dampingFactor=.07;controls.minDistance=.7;controls.maxDistance=90;controls.maxPolarAngle=Math.PI*.48;controls.update();
  ...
  const overviewLabels=new Set(['puerta','altar','fuente','santo','santisimo','cubiertas']);
  function updateWorld(){world.setView({cut,split,walking,isolated:isolated?selected:null});$('cutaway').classList.toggle('active',cut);$('cutaway').setAttribute('aria-pressed',String(cut));$('layers').classList.toggle('active',split);$('layers').setAttribute('aria-pressed',String(split));$('isolate').textContent=isolated?'Ver conjunto':'Aislar 3D';$('isolate').setAttribute('aria-pressed',String(isolated));}
  ...
  function setHeading(){if(!selected){$('scene-heading').textContent='El santuario del desierto';$('scene-description').textContent='Una historia que puedes recorrer.';}}
  function select(id,learn=false) {
    if(!byId[id])return;isolated=false;selected=id;const e=byId[id];
    if(!walking) {
      cut=['santo','santisimo'].includes(e.zone)||['tablas','barras'].includes(id);split=id==='cubiertas';
  ...
  function enterWalk(){
    ...
    ui.toast('Entra por la puerta. Las cortinas se abren para esta visita didáctica.');renderer.domElement.focus({preventScroll:true});
  }
  function setZone(id){lastZone=id;$('location').textContent=id==='exterior'?'DESIERTO DE SINAÍ':zones[id].name.toUpperCase();document.querySelectorAll('[data-zone]').forEach(b=>b.classList.toggle('active',b.dataset.zone===id));}
  function travel(zone){
    stopTour();isolated=false;deselect();
    if(walking){walkTo(zone);clearKeys();updateWorld();return;}
    cut=zone==='santo'||zone==='santisimo';split=false;updateWorld();focus(zones[zone].view,zones[zone].target);setZone(zone);
    if(zone==='santo'||zone==='santisimo'){selected=zone;ui.select(zone);}
  }
  ...
  $('cutaway').onclick=()=>{cut=!cut;isolated=false;updateWorld();};$('layers').onclick=()=>{split=!split;isolated=false;updateWorld();if(split)focus([18,16,5],[0,5,-9]);};
  ...
  function tickWalk(dt){
    ...
    camera.position.y=1.65;camera.rotation.set(pitch,yaw,0,'YXZ');
  ...
  function updateLabels(){
    ...
    for(const e of ordered){
      ...
      else if(!walking&&camera.position.y>13)show=show&&(overviewLabels.has(e.id)||e.id===selected);
      const point=new T.Vector3(...e.point);point.y+=e.id==='cubiertas'?(split?5:1):.8;
  ...
  function animate(){
    ...
    const x=50+camera.position.x*2.8,z=80+camera.position.z*2.8;$('map-view').setAttribute('transform',`translate(${T.MathUtils.clamp(x,3,97)} ${T.MathUtils.clamp(z,3,155)}) rotate(${walking?-yaw*180/Math.PI:0})`);
  ...
  if(new URLSearchParams(location.search).has('debug'))window.tabernacle={getState:()=>({walking,selected,cut,split,isolated,tourIndex,position:camera.position.toArray(),zone:lastZone,canStand:canStand(camera.position.x,camera.position.z),meshes:world.pickables.length,parts:Object.keys(world.groups),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),getBounds:id=>new T.Box3().setFromObject(world.groups[id]).getSize(new T.Vector3()).toArray()};
}
